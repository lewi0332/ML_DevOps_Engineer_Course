...
def test_eda(dff):
    ...
    try:
        assert os.path.getsize('./images/eda/customer_age_distribution.png') > 1
    except FileNotFoundError as err:
        logging.error("FAIL: The customer age distribution image is missing %s", err)
    except AssertionError as err:
        logging.error("FAIL: The customer age distribution image is empty %s", err)
    try:
        assert os.path.getsize('./images/eda/marital_status_distribution.png') > 1
    except FileNotFoundError as err:
        logging.error("FAIL: The marital status distribution image is missing %s", err)
    except AssertionError as err:
        logging.error("FAIL: The marital status distribution image is empty %s", err)
    try:
        assert os.path.getsize('./images/eda/total_transaction_distributio.png') > 1
    except FileNotFoundError as err:
        logging.error("FAIL: The Total Transactions distribution image is missing %s", err)
    except AssertionError as err:
        logging.error("FAIL: The Total Transactions distribution image is empty %s", err)
    try:
...
def test_encoder_helper(dff, path):
    ...
    try:
        assert len(dff_enc.columns) > len(dff.columns)
...
def test_train_models(features):
    ...
    try:
        assert os.path.getsize('./images/results/results/rf_results.png') > 1
    except FileNotFoundError as err:
        logging.error("FAIL: The rf_results.png image is missing %s", err)
    except AssertionError as err:
        logging.error("FAIL: The rf_results.png image is empty %s", err)
    try:
        assert os.path.getsize('./images/results/logistic_results.png') > 1
    except FileNotFoundError as err:
        logging.error("FAIL: The logistic_results.png image is missing %s", err)
    except AssertionError as err:
        logging.error("FAIL: The logistic_results.png image is empty %s", err)
    try:
